...
async function getMusicDetails(title, artist) {
  ...
  // 1. Lấy lyric từ kho NetEase mở không bị chặn
  try {
  ...
  // 2. Tự tìm ID YouTube từ tiêu đề
  try {
...
async function start() {
  ...
  // 4. Cào Top 10 Melon OST Hàn Quốc
  try {
  ...
  // 5. Cào Top 10 Billboard Japan Animation OST
  try {
  ...
  // 6 & 7. Danh sách Nhạc Trung Quốc và OST Trung Quốc tự động quét chi tiết
  ...
  for(let s of baseCn) {
    ...
    freshData.CN_HOT.push({ rank: cIdx++, title: s.t, artist: s.a, ...d });
  }
  ...
  console.log('Đã cập nhật hoàn hảo!');
}
start();
